...
const MovieModal: React.FC<{ movie: Movie; apiKey: string; onClose: () => void; }> = ({ movie, apiKey, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center animate-scale-up-center" onClick={onClose}>
            <div className="bg-zinc-900 rounded-2xl overflow-hidden w-full max-w-4xl h-[90vh] shadow-2xl relative border border-glass-edge mx-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="absolute inset-0 z-10 overflow-y-auto scrollbar-hide">
                    ...
                    <div className="relative z-10 p-8 md:p-12">
                       <div className="h-[45vh]"></div> {/* Spacer */}
                        ...
                        {logoUrl ? (
                            <img src={logoUrl} alt={`${movie.title} logo`} className="max-w-md max-h-40 mb-6" />
                        ) : (
                            <h2 className="text-5xl font-black mb-6">{movie.title}</h2>
                        )}
                        ...
                        {isLoading ? <Loader /> : details && (
                            <div className="space-y-8">
                                ...
                                <p className="text-zinc-300 max-w-3xl">{details.overview}</p>
                                        ...
                                        <div className="flex overflow-x-auto space-x-4 pb-4 scrollbar-hide">
                                            {credits.cast.slice(0, 10).map(member => member.profile_path && (
                                                <div key={member.id} className="flex-shrink-0 w-32 text-center">
                                                    <img src={`https://image.tmdb.org/t/p/w300${member.profile_path}`} alt={member.name} className="rounded-lg shadow-md mb-2 w-full h-44 object-cover"/>
...
const NetflixView: React.FC<NetflixViewProps> = ({ apiKey, searchQuery }) => {
  ...
  return (
    <div className="animate-fade-in">
      {heroMovie && !searchQuery && (
        <div className="relative h-[60vh] -mt-24">
          <div className="absolute inset-0">
            {heroVideoKey ? (
              <div className="absolute inset-0 h-[150%] -top-[25%] pointer-events-none">
                 <VideoPlayer videoKey={heroVideoKey} />
              </div>
            ) : (
          ...
          <div className="absolute bottom-10 left-6 md:left-12 max-w-lg">
            <h1 className="text-4xl md:text-6xl font-black text-white">{heroMovie.title}</h1>
      ...
      {selectedMovie && <MovieModal movie={selectedMovie} apiKey={apiKey} onClose={handleCloseModal} />}
    </div>
  );
};
